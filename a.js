...
import SurveyNotificationModal from '../../components/Modal/SurveyNotificationModal';
...
class SurveyList extends Component {
    ...
    onSelectSinglechoice = (item, indx, survey) => {
        ...
        sQuestions[sIndx].multipleChoice.values.forEach((x, i) => {
    ...
    renderQuestionTypeField(eachSurvey) {
                ...
                if (ratingType === "Star") {
                    return (
                        <div className="col-8 d-flex">
                            <p className="w-20 pt-4">{eachSurvey.rating.minValue}</p>
                            <div className="w-60 align-items-start">
                                <StarRatings
                            ...
                            <p className="w-20 pt-4">{eachSurvey.rating.maxValue}</p>
                        </div>
                    )
                }
        ...
        if(eachSurvey.questionType === "questionmatrix"){
            ...
            if(editAnswersMode && eachSurvey.matrix.rows.length > 0){
                eachSurvey.matrix.rows.forEach((eachrow, rowIndex)=>{
                    eachSurvey.matrix.answers.length > 0 &&
                    eachSurvey.matrix.answers.forEach((eachAns)=>{
    ...
    onSubmitSurvey = () => {
        ...
        finalAnswersArr.length > 0 && finalAnswersArr.forEach((eachSurvey)=>{
            if(eachSurvey.questionType === "questiomatrix"){
                eachSurvey.matrix.selected.length > 0 && eachSurvey.matrix.selected.forEach((eachMatrix)=>{
            ...
            new Api(GetAPIHeader(Storage.getAccessToken())).v31
                ...
                .then((response) => {
                    ...
                    setTimeout(() => {
                        ...
                        this.getSurveyData(1, pageSize, surveyType);
                        $("#myModal").modal("hide");
                    }, 2000)
                ...
                onHide={() => this.checkConfirmationDisplay()}>
                        ...
                        <div className="col-md-12 pt-2 px-0 pb-3 bg-white">
                            <div className="d-flex align-items-center justify-content-between modal-title">
                                {surveyQuestionsData && surveyQuestionsData.surveyStatus==="CANCELLED"? <h6 className="mt-2 ml-4 font-weight-bold xlarge">{t('survey.surveyNotAvailable')}</h6>:<h6 className="mt-2 ml-4 font-weight-bold xlarge">{t('survey.participateInSurvey')}</h6>}
                                <div className="pr-4">
                        ...
                        <div className="pt-2 mr-3">
                            <Modal.Body scrollable="true">
                                {surveyLoading ? <Loading /> :
                                    surveyQuestionsData !== null && (surveyQuestionsData.isSurveyCancelled === true || surveyQuestionsData.surveyStatus==="CANCELLED") ?
    ...
    getSurveyQuestionsWithAnswers(sid) {
        ...
        new Api(GetAPIHeader(Storage.getAccessToken())).v31
            ...
            .then((response) => {
                ...
                let sQuestions = surQueData && surQueData["questions"];
                sQuestions.forEach(qstn => {
                    if (qstn.questionType === "questionmultichoice") {
                        let choiceArr = [];
                        qstn.multipleChoice.values.forEach(choice => {
    ...
    renderButton(item, index) {
        ...
        if (item.selfCreatedSurvey) {
            return (
                ...
                {item.surveyStatus!=="EXPIRED" && item.surveyStatus!=="COMPLETED" && item.isSurveyCreatorCanParticipate && !item.isSurveyCancelled && <button className="button mb-3" disabled={false}
                onClick={() => this.onParticipateClick(item, index)}>
                <p className="text-primary">{t('survey.participateInSurvey')}</p>
                </button>}

                {item.surveyStatus==="COMPLETED" && item.isSurveyCreatorCanParticipate && <div className="d-flex flex-fill mb-3" style={{ wordBreak: 'break-word', overflow: 'hidden' }}>
        ...
        return (
            <div>
                ...
                <div className="row pt-2">
                    <div className="col-md-12">
                        {getIsEnabled(AllFeatures.FE_SURVEY_USER_CAN_EDIT_ANSWER) && <div className="row d-flex align-items-center">
                            ...
                            <div className={surveyList.length>1?"app-card mb-3": "app-card"} key={index}>
                                <div className="container mt-1 survey-wrap">
                                    <div className="d-flex justify-content-between align-items-start">
                                        ...
                                        item.isPendingParticipate && !item.isSurveyCancelled && item.surveyStatus !== "CANCELLED" && item.surveyStatus !== "EXPIRED" &&
                                    ...
                                    <div className="row pt-3 pb-3">
                                        ...
                                        <div className="d-flex col-12 col-lg-4 mt-3 mt-lg-0" style={{ wordBreak: 'break-word', overflow: 'hidden' }}>
                                            ...
                                            <div>
                                                ...
                                                <p className="x-small text-gray">{(item.surveyStatus === "COMPLETED" && !item.isSurveyCancelled) ? t('survey.suveyCompleted') : item.surveyStatus === "EXPIRED" ? t('survey.closed') : (item.surveyStatus === "CANCELLED" || item.isSurveyCancelled) ? t('survey.surveyCancelled') : t('survey.closingDate')}</p>
                                            </div>
                                        </div>
                                    ...
                                    {!item.isSurveyCancelled && item.surveyStatus === "COMPLETED" && this.renderEditContainer(item)}
                                </div>
                            </div>
                        )
                    })
        ...
        return (
            loading ? <Loading Height="0" /> :
                <React.Fragment>
                    ...
                    {renderNotificationModal && <SurveyNotificationModal 
                            show={true}
                            handleClose={this.handleNotificationModalClose.bind(this)}
